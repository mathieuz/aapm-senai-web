...
CHAVE_DE_PESQUISA = re.compile(r"DOCUMENTO FISCAL - GIA", flags = re.M | re.I)
DATA_REGEX = re.compile(r"Termos de Uso[\s\t\n\d\w]*versão[\s\t\n\d\.\-]*\d{2}/(\d{2})/(\d{4})")
    ...
    if ".pdf" in arquivo:
        ...
        for i in range (0, len(arquivoPdf)):
            ...
            if CHAVE_DE_PESQUISA.findall(pagina):
                print(f"[!!!] ENCONTRADO - {arquivo} - [{CHAVE_DE_PESQUISA.findall(pagina)}]")
