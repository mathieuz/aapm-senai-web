...
NOME_DECLARACAO = "Nos conformes"

### Regexes de busca nos arquivos (chave de pesquisa e todas as datas possíveis encontradas) ###
CHAVE_DE_PESQUISA = re.compile(r"DETALHES DA CLASSIFICAÇÃO POR CRITÉRIO", flags = re.M | re.I)
DATA_REGEX = re.compile(r"[\s\t\w\d\n]*Período Avaliado:[\d\/\w\s\t]*(\d{2})/(\d{4})", flags = re.M | re.I)
