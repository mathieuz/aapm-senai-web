...
NOME_DECLARACAO = "DCTFweb"

### Regexes de busca nos arquivos (chave de pesquisa e todas as datas possíveis encontradas) ###
CHAVE_DE_PESQUISA = re.compile(r"DCTFWeb", flags = re.M | re.I)
DATA_REGEX = re.compile(r"Período de apuração (\d{2})/(\d{4})|[\s\t\n\d\w]*Período apuração[\s\t\n\w]*(\d{2})/(\d{4})", flags = re.M | re.I)
            ...
            if CHAVE_DE_PESQUISA.findall(pagina):
                ...
                for j in range (0, len(arquivoPdf)):
                    ...
                    if DATA_REGEX.findall(paginaData):
                        ...
                        # Há mais de um regex de data para este arquivo especifico.
                        # Essas possibilidades são testadas.
                        mesData = DATA_REGEX.findall(pagina)[0][0]
                        anoData = DATA_REGEX.findall(pagina)[0][1]

                        if (mesData == "" or anoData == ""):
                            mesData = DATA_REGEX.findall(pagina)[0][2]
                            anoData = DATA_REGEX.findall(pagina)[0][3]
