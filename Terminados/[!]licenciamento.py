...
NOME_DECLARACAO = "Licenciamento integrado"

### Regexes de busca nos arquivos (chave de pesquisa e todas as datas possíveis encontradas) ###
CHAVE_DE_PESQUISA = re.compile(r"LICENCIAMENTO INTEGRADO", flags = re.M | re.I)
DATA_REGEX = re.compile(r"DATA DA SOLICITAÇÃO[\w\s\t\n]*\d{2}\/(\d{2})\/(\d{4})", flags = re.M | re.I)
